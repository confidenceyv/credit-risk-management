...
query = "select * from stg.runnig_loans_cust_corp"
...
pd_df['Outflow_Volume_Bin'] = pd.cut(pd_df['1YR_DR_TRAN_COUNT'],
                            bins=[-7,-6,-5,-4,-3,-2,-1,(max(pd_df['1YR_DR_TRAN_COUNT']))],
...
cut_labels = [1, 2, 3, 4,5,6,7,8,9]
cut_bins = [-1000000,-500000, -250000, -100000, -50000, -10000,-1000, -100, -10, (max(pd_df['1YR_DR_AMT']))]
...
.option("tempDir",temp_dir_url)\
.option("forward_spark_azure_storage_credentials", "true")\
.option("dbtable", "stg.LD_proc_predict_set_corp")\
.mode("overwrite")\
.save()
