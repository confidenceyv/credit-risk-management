...
query = "select * from stg.[loan_dataset_due_cust_corp]"
...
pd_df['Inflow_Volume_Bin'] = pd.cut(pd_df['1YR_CR_TRAN_COUNT'],
                            bins=[-1,2, 4, 6, 8, 10, (max(pd_df['1YR_CR_TRAN_COUNT']))],
                            labels=[1,2,3,4,5,6])
...
pd_df['Outflow_Volume_Bin'] = pd.cut(pd_df['1YR_DR_TRAN_COUNT'],
                            bins=[-1,2, 3, 4, 5, 6, 7, (max(pd_df['1YR_DR_TRAN_COUNT']))],
...
cut_labels = [1, 2, 3, 4,5,6,7,8,9]
cut_bins = [-1,0, 25000, 50000, 100000, 150000,200000,250000,300000,(max(pd_df['1YR_DR_AMT']))]
...
corr_cols = []
for i in pd_df.select_dtypes(['int32','float64']):
  corr_cols.append(i)
corr_cols.append('Loan_Status')

# COMMAND ----------

k = len(corr_cols)
cols = pd_df[corr_cols].corr().nlargest(k, 'Loan_Status')['Loan_Status'].index
cm = pd_df[cols].corr()
...
.option("tempDir",temp_dir_url)\
.option("forward_spark_azure_storage_credentials", "true")\
.option("dbtable", "stg.LD_proc_train_set_corp")\
.mode("overwrite")\
.save()
