...
query = "select * from stg.LD_proc_train_set_corp"
...
# with mlflow.start_run(experiment_id = 634118024211424):
#   n_estimators = 100
...
#querying data in SQL datawarehouse
query = "select * from stg.LD_proc_predict_set_corp"
...
pred_result = xgb.predict(df_features)
pred_probability = xgb.predict_proba(df_features)
final_df['probability_of_default'] = pred_probability[:, 1].reshape(-1, 1)
final_df['predicted_default_flag'] = pred_result


# COMMAND ----------

final_df = final_df[['COUNTRY', 'CUSTOMER_ID','LOAN_ACCT', 'predicted_default_flag' , 'probability_of_default']]

# COMMAND ----------

final_df.predicted_default_flag.value_counts()
...
.option("tempDir",temp_dir_url)\
.option("forward_spark_azure_storage_credentials", "true")\
.option("dbtable", "stg.Loan_default_predictions_corp")\
.mode("overwrite")\
.save()
