...
pd_df['1YR_CR_TRAN_COUNT_BIN'] = pd.cut(pd_df['1YR_CR_TRAN_COUNT'],
                            bins=[-1,0, 3, 6, 8, 10, (max(pd_df['1YR_CR_TRAN_COUNT']))],
                            labels=[1,2,3,4,5,6])
...
pd_df['1YR_DR_TRAN_COUNT_BIN'] = pd.cut(pd_df['1YR_DR_TRAN_COUNT'],
                            bins= [-1,0, 1, 2, 4, 6, (max(pd_df['1YR_DR_TRAN_COUNT']))],
                            labels=[1,2,3,4,5,6])
...
pd_df['F6M_CR_TRAN_COUNT_BIN'] = pd.cut(pd_df['F6M_CR_TRAN_COUNT'],
                            bins= [-1,0, 1, 3, 4,  (max(pd_df['F6M_CR_TRAN_COUNT']))],
                            labels=[1,2,3,4,5])
...
pd_df['L6M_CR_TRAN_COUNT_BIN'] = pd.cut(pd_df['L6M_CR_TRAN_COUNT'],
                            bins=[-1,0, 1, 2, 4,  (max(pd_df['L6M_CR_TRAN_COUNT']))],
                            labels=[1,2,3,4,5])
...
pd_df['F6M_DR_TRAN_COUNT_BIN'] = pd.cut(pd_df['F6M_DR_TRAN_COUNT'],
                            bins=[-1,0, 1, 2, 3,  (max(pd_df['F6M_DR_TRAN_COUNT']))],
                            labels=[1,2,3,4,5])
...
.option("tempDir",temp_dir_url)\
.option("forward_spark_azure_storage_credentials", "true")\
.option("dbtable", "stg.NHC_proc_predict_set_corp")\
.mode("overwrite")\
.save()
