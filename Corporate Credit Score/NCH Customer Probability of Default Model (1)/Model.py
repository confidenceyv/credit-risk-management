...
query = "select * from stg.NHC_proc_predict_set_corp"
...
X = pd_df.drop(['Loan_Status','CUSTOMER_ID', 'COUNTRY','account_no'], axis=1)
#X = pd.get_dummies(X)
...
un_X = pd_df.drop(['Loan_Status','CUSTOMER_ID', 'COUNTRY','account_no'], axis=1)
#X = pd.get_dummies(X)
un_y = pd_df['Loan_Status']
un_X_train, un_X_test, un_y_train, un_y_test = train_test_split(un_X, un_y, test_size=0.2, random_state=155)
...
un_rf_model = RandomForestClassifier(max_depth=5, n_estimators=100)
...
df_features = final_df.drop(['CUSTOMER_ID', 'COUNTRY','account_no','Loan_Status'], axis=1)
...
final_df = final_df[['COUNTRY', 'CUSTOMER_ID' , 'account_no', 'probability_of_repayment']]
...
.option("tempDir",temp_dir_url)\
.option("forward_spark_azure_storage_credentials", "true")\
.option("dbtable", "stg.nhc_probability_of_repayment_corp")\
.mode("overwrite")\
.save()
